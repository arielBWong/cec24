...
FU_infeasible = FU(~idx, :);
scatter(pf(:, 1), pf(:, 2), 10, 'k', 'filled',  'DisplayName','UL Pareto front'); hold on;
scatter(FU_feasible(:, 1), FU_feasible(:, 2), 20, 'red', 'filled', 'DisplayName','Feasible LL PS solutions');
scatter(FU_infeasible(:, 1), FU_infeasible(:, 2), 10, 'red', 'DisplayName','Infeasible LL PS solutions');
title('UL PF/LL PS solutions', 'FontSize', 10);

box on;
grid on;


savename = sprintf("%s_UL_pf_and_llsearch", prob.name);
